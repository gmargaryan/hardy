...
echo $JAVA_VERSION_RAW | grep 'version[[:space:]]*\("1\.[678]\.[0-9]*_[0-9]*"\|"9"\|"\d{2}[.-].*"\)' 2>&1 > /dev/null
...
if [ $? -eq "1" ]; then
        ...
        echo ""
        echo "You must use Java 1.6.x or higher. The most recent version of Java is recommended."        
        exit 1;
fi
